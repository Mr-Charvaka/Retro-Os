...
ntfs_context_t *ntfs_mount(uint8_t drive_id, uint32_t partition_start_lba) {
    uint8_t buffer[512];
    if (!block_read(drive_id, (uint64_t)partition_start_lba, 1, buffer)) return nullptr;
...
static bool ntfs_read_mft_record(ntfs_context_t *ctx, uint64_t index, uint8_t *buffer) {
    ...
    for (uint32_t i = 0; i < sectors; i++) {
        if (!block_read(ctx->drive_id, (uint64_t)(sector + i), 1, buffer + (i * 512))) return false;
    }
    ...
    if (!data_attr->non_resident) {
        ...
        for (int i = 0; i < run_count; i++) {
            if (current_vcn + runs[i].cluster_count > vcn_needed) {
                ...
                block_read(ctx->drive_id, (uint64_t)abs_sector, 1, cluster_buf); // Actually read 1 sector for now
