...
void pmm_mark_region_used(uint32_t base, uint32_t size) {
  ...
  for (uint32_t b = 0; b < blocks; b++) {
    ...
    if (frame >= pmm_max_blocks)
      break; // Past end of managed memory
    if (!mmap_test(frame)) {
      mmap_set(frame);
      pmm_used_blocks++;
    }
...
void pmm_mark_region_free(uint32_t base, uint32_t size) {
  ...
  for (uint32_t b = 0; b < blocks; b++) {
    uint32_t frame = align + b;
    if (frame == 0)
      continue; // Never free the IVT frame
    if (frame >= pmm_max_blocks)
      break;
    if (mmap_test(frame)) {
      mmap_unset(frame);
 ...
 *   • Double-validates the found frame is actually free (bitmap corruption
 * guard) • Validates returned address is < pmm_mem_size
 *
