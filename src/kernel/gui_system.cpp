...
void launch_brain_activation() {
  g_brain_activated = 1;
  serial_log("GUI: AI Brain Activation Signal SENT.");
}
}
...
/* 🖼️ ASSET LOADER (Wallpaper ke liye) */
...
struct BMPInfoHeader {
    ...
    uint32_t image_size;
    int32_t x_pels_per_meter, y_pels_per_meter;
    uint32_t clr_used, clr_important;
};
#pragma pack(pop)

static uint32_t* g_wallpaper = nullptr;
static int g_wall_w = 0, g_wall_h = 0;
...
void load_wallpaper(const char* path) {
    ...
    BMPHeader h;
    BMPInfoHeader ih;
    if (sys_read(fd, &h, sizeof(h)) <= 0) { sys_close(fd); return; }
    if (sys_read(fd, &ih, sizeof(ih)) <= 0) { sys_close(fd); return; }

    if (h.type != 0x4D42) { sys_close(fd); return; }
...
void refresh() {
  ...
  // Rev S: Add Fixed AI Activation Icon
  add_or_update("Activate AI", IconSystem::ICON_COMPUTER, launch_brain_activation);
}


void draw_desktop() {
  // 1. Wallpaper or Gradient background
  if (AssetLoader::g_wallpaper) {
      for (int y = 0; y < FB::H; y++) {
          if (y >= AssetLoader::g_wall_h) break;
          for (int x = 0; x < FB::W; x++) {
              if (x >= AssetLoader::g_wall_w) break;
              FB::put(x, y, AssetLoader::g_wallpaper[y * AssetLoader::g_wall_w + x]);
          }
      }
  } else {
      for (int y = 0; y < FB::H; y++) {
  ...
  // Draw Diagnostic Square (Phase 19 Verification)
  FB::rect(10, 10, 20, 20, 0xFFFF0000); 
...
/* Browser and Dillo implementations removed */
...
void sysmon_draw(Window *w) {
  ...
  *p = 0;
  ...
  FontSystem::draw_text(font, label_x, line_y, "Processors:", 0xCCCCCC);
  char cpu_buf[16];
  cpu_buf[0] = '0' + (total_cpus % 10);
  cpu_buf[1] = 0;
  if (total_cpus >= 10) {
      cpu_buf[0] = '0' + (total_cpus / 10);
      cpu_buf[1] = '0' + (total_cpus % 10);
      cpu_buf[2] = 0;
  }
  FontSystem::draw_text(font, value_x, line_y, cpu_buf, 0x4FC3F7);
  line_y += 14;
...
extern "C" void gui_main() {
  // DO NOT call bga_set_video_mode here — already set in kernel.cpp!
  
  serial_log("GUI: gui_main entry...\n");
  FB::init();
  
  if (!FB::buf) {
      serial_log("GUI: FATAL — Framebuffer is NULL!");
      return;
  }
  
  serial_log("GUI: Framebuffer initialized.");
  serial_log_hex("GUI: FB buf = ", (uint32_t)(uintptr_t)FB::buf);
  serial_log_hex("GUI: FB W = ", FB::W);
  serial_log_hex("GUI: FB H = ", FB::H);

  // Diagnostic Test Pattern
  for (int y = 100; y < 200; y++) {
      for (int x = 100; x < 300; x++) {
          FB::put(x, y, 0xFFFF0000);
      }
  }
  FB::swap();
  
  serial_log("GUI: Starting DesktopSystem::refresh...\n");
  AssetLoader::load_wallpaper("/C/WALL.BMP");
  DesktopSystem::refresh();
  serial_log("GUI: Initializing WindowServer...\n");
  WindowServer::init();
  serial_log("GUI: Launching explorer...\n");
  launch_explorer();
  ...
  while (true) {
      ...
      if (g_rename.active) {
        ...
        for (int i = win_count[current_desktop] - 1; i >= 0; i--) {
          if (windows[current_desktop][i].alive &&
            ...
            windows[current_desktop][i].key(&windows[current_desktop][i], k, ks);
            break;
          }
        }
      }
    ...
    // Window cleanup (reap dead windows)
    ...
    static uint32_t last_heartbeat_tick = 0xFFFFFFFF;
    if (tick % 500 == 0 && tick != last_heartbeat_tick) {
        serial_log_hex("GUI: Heartbeat [OK] - Ticks: ", tick);
        last_heartbeat_tick = tick;
    }

    FB::swap();
    kernel_yield(); // Give other threads time
  }
