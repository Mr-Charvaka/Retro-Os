...
static Renode *newrep(struct cstate *g, Renode *atom, int ng, int min,
                      int max) {
...
static void next(struct cstate *g) { g->lookahead = lex(g); }
...
static Renode *parserep(struct cstate *g) {
  ...
  if (accept(g, '*'))
    return newrep(g, atom, accept(g, '?'), 0, REPINF);
  if (accept(g, '+'))
    return newrep(g, atom, accept(g, '?'), 1, REPINF);
  if (accept(g, '?'))
    return newrep(g, atom, accept(g, '?'), 0, 1);
  return atom;
}
...
void regfree(Reprog *prog) { regfreex(default_alloc, NULL, prog); }
...
static int match(Reinst *pc, const char *sp, const char *bol, int flags,
                 Resub *out, int depth) {
  Resub scratch;
  int result;
  int i;
        ...
        for (i = 0; i < m.nsub; ++i) {
          ...
          if (n > 0)
            printf("match %d: s=%d e=%d n=%d '%.*s'\n", i,
                   (int)(m.sub[i].sp - s), (int)(m.sub[i].ep - s), n, n,
                   m.sub[i].sp);
          else
            printf("match %d: n=0 ''\n", i);
        }
