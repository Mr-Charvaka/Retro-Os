...
uint32_t load_elf(const char *filename, uint32_t *top_address) {
    ...
    if (phdrs[i].p_type == PT_LOAD) {
      ...
      for (uint32_t page = start_page; page < end_page; page += 4096) {
        if (vm_get_phys(page) == 0) {
          ...
          vm_map_page((uint32_t)(uintptr_t)phys, page, 7); // User/RW (Flags 7)
          memset((void*)(uintptr_t)page, 0, 4096); // Zero initialised
        }
      }

      // Read segment data directly from disk into its virtual address
      if (file_size > 0) {
        int read_bytes = vfs_read(node, phdrs[i].p_offset, (void*)(uintptr_t)start_addr, file_size);
        if (read_bytes < (int)file_size) {
            serial_log_hex("ELF FATAL: Segment read failed. Expected ", file_size);
            serial_log_hex("  Actually read: ", read_bytes);
            kfree(phdrs);
            return 0;
        }
      }

      // Zero the BSS section (the part of MemSiz not in FileSiz)
      if (size > file_size) {
        memset((void*)(uintptr_t)(start_addr + file_size), 0, size - file_size);
      }
    }
  }
  ...
  kfree(phdrs);
  serial_log_hex("ELF: Successfully loaded. Entry at ", ehdr.e_entry);
  return ehdr.e_entry;
}
