...
extern "C" int e1000_receive(uint8_t *out) {
  ...
  // Periodic debug DISABLED to reduce log spam
  // Uncomment to debug RX ring issues:
  // static int poll_count = 0;
  // if (++poll_count >= 500000) {
  //   poll_count = 0;
  //   serial_log_hex("e1000: RDH=", e1000_read(E1000_RDH));
  // }
  ...
  // Calculate the OLD tail value (what we're giving back)
  uint32_t old_rx_tail = rx_tail;

  // Advance our software pointer first
  rx_tail = (rx_tail + 1) % RX_DESC_COUNT;

  // Now update RDT to include the processed descriptor
  // RDT should point to the descriptor we just processed (giving it back)
  // This makes the ring: hardware owns from (RDT+1) to (RDH-1)
  // But we want to keep hardware owning as many as possible
  // So we set RDT to one BEFORE the new rx_tail
  uint32_t new_rdt = (rx_tail + RX_DESC_COUNT - 2) % RX_DESC_COUNT;
  e1000_write(E1000_RDT, new_rdt);
