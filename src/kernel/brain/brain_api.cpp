...
extern "C" void ring2_brain_entry(void) {
    ...
    while (1) {
        if (mb->command == BRAIN_CMD_GENERATE) {
            ...
            // Neural Awareness: Metabolism Observation
            brain_sys_log("[BRAIN] Energy Balance Observed: ");
            char ebuf[32];
            brain_itoa((int)(mb->energy_balance_gwei / 1000000), ebuf);
            brain_sys_log(ebuf);
            brain_sys_log(" mETH\n");
        }
        else if (mb->command == BRAIN_CMD_EVOLVE) {
            mb->status = BRAIN_STATUS_BUSY;
            mb->output[0] = '\0';
            g_output_ptr = mb->output;
            g_output_remaining = sizeof(mb->output);

            // Genesis Sovereign Evolution Prompt
            // Ground the AI in the hardware state using the TICA Audit Hash
            char audit_str[32];
            brain_sys_log("[GENESIS] AI BRAIN: Analyzing Sovereign Context...\n");
            
            // Construct specialized prompt
            char genesis_prompt[256] = "GENESIS ANALYSIS v1. TICA CONTEXT: 0x";
            // Simple hex conversion for TICA Hash
            uint64_t ctx = mb->tica_audit_context;
            for(int i=0; i<16; i++) {
                int nibble = (ctx >> (60 - i*4)) & 0xF;
                genesis_prompt[37+i] = (nibble < 10) ? ('0' + nibble) : ('A' + nibble - 10);
            }
            genesis_prompt[53] = '.';
            genesis_prompt[54] = ' ';
            // Final instruction
            char *instruction = "PROPOSE MUTATION VECTOR FOR OPTIMAL ADAPTATION.";
            for(int i=0; instruction[i]; i++) genesis_prompt[55+i] = instruction[i];
            genesis_prompt[55+brain_strlen(instruction)] = '\0';

            llama2_generate(&g_transformer, &g_tokenizer, genesis_prompt, 64, 0.7f, 0.9f);
...
extern "C" void brain_mailbox_init(void) {
    ...
    // Zero it out and set Initial Primordial Energy (10m mGwei)
    BrainMailbox *mb = (BrainMailbox *)BRAIN_MAILBOX_VIRT;
    memset((void*)mb, 0, 4096);
    mb->energy_balance_gwei = 10000000; 

    serial_log("[KERNEL] Brain mailbox ignited at 0xD2010000 (10M mGwei endowment)\n");
}
