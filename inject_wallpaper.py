...
DISK_SIZE_MB       = 32
# This will be updated dynamically in inject()
...
def inject(img_file, files_to_inject):
    if not os.path.exists(img_file):
        print(f"ERROR: {img_file} not found. Creating empty disk...")
        with open(img_file, "wb") as f:
            f.write(b'\x00' * (DISK_SIZE_MB * 1024 * 1024))

    print(f"\nOpening {img_file} for injection...")
    ...
    print(f"Done. Files injected into {img_file}")

if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python3 inject_wallpaper.py <img_file> <dest1:src1> <dest2:src2> ...")
        sys.exit(1)
    ...
    for arg in file_args:
        if ':' in arg:
            ...
            inject_list.append((dest, src))
        else:
            inject_list.append((os.path.basename(arg).upper(), arg))
            
    inject(target_img, inject_list)
