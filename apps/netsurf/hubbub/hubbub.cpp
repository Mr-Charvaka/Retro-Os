...
hubbub_error hubbub_parser_parse_chunk(hubbub_parser *parser, const uint8_t *data, size_t len) {
    ...
    /* Example logic: Tokenize very basic tags */
    ...
    while (p < end) {
        if (*p == '<') {
            ...
            if (*p == '/') {
                ...
                void *node = NULL;
                if (parser->handler.create_element) {
                    parser->handler.create_element(parser->ctx, tag, &node);
                    printf("DEBUG: Hubbub created element for tag %p (node=%p, ctx=%p)\n", (void*)tag, node, parser->ctx);
                }

                if (node && parser->handler.append_child) {
                    printf("DEBUG: Hubbub appending child %p to parent %p (ctx=%p)\n", node, parser->current_node, parser->ctx);
                    parser->handler.append_child(parser->ctx, parser->current_node, node);
